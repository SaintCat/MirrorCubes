...
class Renderer implements GLEventListener {
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl.glShadeModel(GL.GL_FLAT); // Select Flat Shading (Nice Definition Of
										// Objects)
	...
	void torus(GL gl, float MinorRadius, float MajorRadius) // Draw A Torus With
															// Normals
	{
		int i, j;
		...
		for (i = 0; i < 20; i++) // Stacks
		{
			for (j = -1; j < 20; j++) // Slices
			{
				...
				gl.glNormal3d((Math.sin(Math.PI * 2.0 * (i % 20 + wrapFrac)
						/ (float) 20))
						* cosphi, sinphi, (Math.cos(Math.PI * 2.0
						* (i % 20 + wrapFrac) / (float) 20))
						* cosphi);
				gl.glVertex3d((Math.sin(Math.PI * 2.0 * (i % 20 + wrapFrac)
						/ (float) 20))
						* r, MinorRadius * sinphi, (Math.cos(Math.PI * 2.0
						* (i % 20 + wrapFrac) / (float) 20))
						* r);

				gl.glNormal3d((Math.sin(Math.PI * 2.0 * (i + 1 % 20 + wrapFrac)
						/ (float) 20))
						* cosphi, sinphi, (Math.cos(Math.PI * 2.0
						* (i + 1 % 20 + wrapFrac) / (float) 20))
						* cosphi);
				gl.glVertex3d((Math.sin(Math.PI * 2.0 * (i + 1 % 20 + wrapFrac)
						/ (float) 20))
						* r, MinorRadius * sinphi, (Math.cos(Math.PI * 2.0
						* (i + 1 % 20 + wrapFrac) / (float) 20))
						* r);
